...
// Public services get added to the load balancer
resource "aws_ecs_service" "ui" {
  ...
  depends_on = [aws_ecs_cluster.main, aws_ecs_task_definition.ui]

  load_balancer {
    target_group_arn = var.target_group_ui_arn
    container_name   = "ui"
    container_port   = 80
  }
...
resource "aws_ecs_service" "consul" {
  ...
  depends_on = [aws_ecs_cluster.main, aws_ecs_task_definition.consul]

  network_configuration {
    subnets         = [var.subnet_pvt01_id, var.subnet_pvt02_id]
    security_groups = [var.sg_allow_consul_id]
  }
